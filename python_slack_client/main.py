...
"""
    ...
    Source code: https://github.com/terraform-aws-modules/terraform-aws-notify-slack/blob/master/functions/notify_slack.py
"""
...
def format_cloudwatch_alarm(message: Dict[str, Any], region: str) -> Dict[str, Any]:
    ...
    if message["NewStateValue"] == "OK":
        alarm_description = "Everything is back to normal"
        response["fields"].extend(
            [
                {
                    "title": "Alarm Description",
                    "value": f"`{alarm_description}` :white_check_mark:",
                    "short": False,
                }
            ]
        )
